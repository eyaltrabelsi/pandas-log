...
def dq_check(df, percentiles=None, thorough=True):
    ...
    mode = (
        ...
        if "mode" in description.index
        else df.mode().iloc[0]
    )
    # TODO: Possibly faster to just calculate description.value_counts()?
    mode_freq = (
        description.loc["mode_freq"]
        if "mode_freq" in description.index
        else (df == mode).sum()
    )
    ...
    n_unique = (
        description.loc["n_unique"]
        if "n_unique" in description.index
    ...
    if thorough:
        ...
        if not strings.empty:
            edge_whitespace = strings.apply(
                lambda col: col.str.match(r"^\s+.*|.*\s+$", na=False).sum()
            )
            edge_whitespace = pd.DataFrame(
                [edge_whitespace], index=["has_edge_whitespace"]
            )
            thorough_df = pd.concat(
                [thorough_df, edge_whitespace], axis=1, sort=True
            )
    else:
        thorough_df = pd.DataFrame([])
